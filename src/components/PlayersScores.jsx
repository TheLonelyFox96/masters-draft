              ...
              <table className="scores-table">
                ...
                <tbody>
                  {pickedPlayers.map((player) => {
                    ...
                    return (
                      <tr key={player.id} className={player.missedCut ? 'row--missed-cut' : ''}>
                        <td className="scores-table__name">
                          ...
                          />
                          {player.name}
                          {player.missedCut && <span className="badge--mc">MC</span>}
                        </td>
                        <td>{formatRound(player.rounds[0], player.missedCut, 0)}</td>
                        <td>{formatRound(player.rounds[1], player.missedCut, 1)}</td>
                        <td>{formatRound(player.rounds[2], player.missedCut, 2)}</td>
                        <td>{formatRound(player.rounds[3], player.missedCut, 3)}</td>
                        <td className={`scores-table__total ${total < 0 ? 'score--under' : total > 0 ? 'score--over' : 'score--even'}`}>
                          {formatScore(total)}
                        </td>
                      </tr>
                    );
